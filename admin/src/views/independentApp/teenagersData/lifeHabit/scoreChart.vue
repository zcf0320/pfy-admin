...
import option from '../components/scoreChart';
export default {
  name: 'allData',
  props: {
    tab: {
  ...
  methods: {
    initChart() {
      ...
      if (this.tab) {
        option.color = ['#6CDFAD'];
        option.yAxis.axisLabel.formatter = '{value}h';
        option.series[0].name = '时长';
      } else {
        option.color = ['#6395F9'];
        option.yAxis.axisLabel.formatter = '{value}分';
        option.series[0].data = '得分';
      }
