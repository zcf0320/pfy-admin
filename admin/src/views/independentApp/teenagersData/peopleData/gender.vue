<template>
  <div class="user-gender">
    ...
    <div class="user-gender-box flex">
      <div class="user-gender-left">学生性别分布比例</div>
    ...
    <Bar v-bind="processBarModel" :people-data="peopleData" />
  </div>
</template>
    ...
    data: {
      handler(n) {
        if (n) {
          ...
          this.processBarModel.processValue.push(
            ((male / total) * 100).toFixed(0),
            ((female / total) * 100).toFixed(0),
            ((unknow / total) * 100).toFixed(0)
          );
          this.peopleData = [male, female, unknow];
        }
      },
      deep: true,
      immediate: true
