  ...
  methods: {
    ...
    getList(pageInfo) {
      ...
      const params = {
        ...
        diagnoseStartTime: vm.filter.filterTime.length ? vm.filter.filterTime[0] : '',
        diagnoseEndTime: vm.filter.filterTime.length ? vm.filter.filterTime[1] : ''
      };
      ...
      this.$store
        .dispatch('workOrderManage/getList', params)
        .then(res => {
    ...
    // 查看详情
    lookDetails(id) {
      this.$router.push(`/inquiryManage/workOrderManage/detail/${id}`);
    },
    // 打开聊天工具页面
    goChat() {
    ...
    // 获取医生的工单数据
    getDayMgs() {
      this.$store.dispatch('workOrderManage/getDayMgs', 0).then(res => {
