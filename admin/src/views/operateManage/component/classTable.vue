  ...
  methods: {
    ...
    fieldCharacter(weekIndex, courseIndex) {
      if (
        ...
        // data.map((item) => {
        html += `<div class="item-name">${data[0].name}</div><div class="content">${data[0].content}</div>`;
        // })
    ...
    initData() {
      ...
      this.$store.dispatch('userManage/getDurgPlanDetail', params).then(res => {
        let data = res.data;
        if (data) {
          data.forEach(item => {
