        ...
        <div class="details-content">
                        ...
                        <el-row :gutter="20">
                          ...
                          <el-col :span="24">
                            <el-form-item label="简介" prop="introduce">
                              ...
                              <!-- <InputMaxTip
                                :max="40"
                                :current-key="contentForm.introduce"
                              /> -->
                            </el-form-item>
                          </el-col>
                          <el-col :span="24">
                          ...
                          <el-col
                            v-if="
                              checked || (contentForm.employeeEmailVOList&&contentForm.employeeEmailVOList.length)
                            "
                            :span="24"
                          >
          ...
          <div v-else class="content flex-col">
            <div class="content-top">
              ...
              <div class="details">
                <span>{{
                  `${contentType[contentForm.type[0]]}-${
                    contentType[contentForm.type[1]]
                  }`
                }}</span>
                <span class="right">
            ...
            <div v-if="contentForm.advertisVideo">
              <video :src="contentForm.advertisVideo" controls class="video" />
            </div>
            <div id="text" class="text scroll-table" />
          </div>
        </div>
        <div v-if="type === '1'" class="page-footer">
          ...
          <button
            v-if="$route.params.postStatus !== '105'"
            type="button"
...
export default {
  ...
  mounted() {
    ...
    editor.create();
    if (
      this.$route.params.postStatus &&
      this.$route.params.postStatus === '105'
    ) {
  ...
  methods: {
    ...
    preview() {
      if (this.qrcode) {
        ...
        this.qrcode.makeCode(
          `${process.env.VUE_APP_USER_CENTER}/#/article/detail/index?id=${
            this.qrcodeID
    ...
    uploadPropogandaPic(item) {
      uploadImage(item)
        .then(res => {
          this.contentForm.advertisPic = res;
        })
        .catch(() => {
          this.$message.error('上传图片有误');
        });
    ...
    updateContent(formName) {
      ...
      vm.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          vm.loading = true;
          vm.$store
            .dispatch('contentManage/updateContent', vm.contentForm)
            .then(res => {
              if (res.code === 0) {
    ...
    getDetails(id) {
      ...
      this.$store
        ...
        .then(res => {
          this.contentForm = res.obj;
          if (this.type === '1') {
            editor.txt.html(this.contentForm.content);
          } else {
            document.getElementById('text').innerHTML = this.contentForm.content;
          }
          if (res.obj.fileUrls) {
            this.fileUrls = res.obj.fileUrls.split(',');
          }
          this.peopleList = res.obj.employeeEmailVOList;
          if (this.peopleList && this.peopleList.length) {
            this.checked = true;
          }
          this.typeChange(vm.contentForm.type);
        });
    },
    // 分类改变获取标签
    typeChange(ids) {
    ...
    publishContent(postStatus) {
      ...
      this.$store
        .dispatch('contentManage/publishContent', params)
        .then(() => {
          if (this.checked && postStatus !== '106') {
            this.send();
          } else {
            this.$router.go(-1);
            this.$message.success(
