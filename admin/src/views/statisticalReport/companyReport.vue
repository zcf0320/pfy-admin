      ...
      >
        <div class="policy-service">
          <div class="policy-service-title">
            保险公司：{{ data.insuranceCompanyName || '-' }}
          </div>
          <div
            v-for="(item, index) in data.reportFormInsuranceProductVOList"
            :key="`保险公司1${index}`"
            class="company-item"
          >
            <div class="policy-info-text">{{ item.insuranceProductName }}</div>
            <div class="policy-service-money">
              <div class="policy-service-money-l">
                <div class="company-buy-l">
                  <div class="company-buy-img1" />
                  <div class="company-buy-num">{{ item.saleNumber }}</div>
                  <div class="company-buy-text">保单销量</div>
                </div>
                <div class="company-buy-r">
                  <div class="company-buy-l">
                    <div class="company-buy-img2" />
                    <div class="company-buy-num">{{ item.premium }}</div>
              ...
              <div class="policy-service-money-r">
                <!-- <div class="claim-percentage-item">
                                    <div class="claim-percentage">
                                        <div class="flex-between">
                                            <div class="title">保单结算</div>
                                            <div class="money-num"> {{item.policeSettlementPriceComplete}}</div>
                                        </div>
                                        <el-progress
                                            :percentage="item.per1"
                                            :stroke-width="8"
                                            color="#3599FE"
                                            :show-text="false"
                                        ></el-progress>
                                    ...
                                    <div class="claim-percentage-num">{{item.policeSettlementPriceWait}}</div>
                                </div>
                                <div class="claim-percentage-item">
                                    <div class="claim-percentage">
                                        <div class="flex-between">
                                            <div class="title">理赔结算</div>
                                            <div class="money-num"> {{item.claimSettlementPriceComplete}}</div>
                                        </div>
                                        <el-progress
                                            :percentage="item.per2"
                                            :stroke-width="8"
                                            color="#3599FE"
                                            :show-text="false"
                                        ></el-progress>
                                    ...
                                    <div class="icon-money"></div>
                                    <div class="claim-percentage-text">待结算</div>
                                    <div class="claim-percentage-num">{{item.claimSettlementPriceWait}}</div>
                                </div>
                                <div class="claim-percentage-item">
                                    <div class="claim-percentage">
                                        <div class="flex-between">
                                            <div class="title">供应商结算</div>
                                            <div class="money-num"> {{item.supplierSettlementPriceComplete}}</div>
                                        </div>
                                        <el-progress
                                            :percentage="item.per3"
                                            :stroke-width="8"
                                            color="#3599FE"
                                            :show-text="false"
                                        ></el-progress>
                                    ...
                                    <div class="icon-money"></div>
                                    <div class="claim-percentage-text">待结算</div>
                                    <div class="claim-percentage-num">{{item.supplierSettlementPriceWait}}</div>
                                </div> -->
                ...
                <div class="service-money-r-item mb-50">
                  ...
                  <div class="service-money-r-item-b mb-50">
                    ...
                    <div class="claim-percentage-text">
                      已结算:{{ item.policeSettlementPriceComplete || '-' }}
                    </div>
                    <div class="icon-money" />
                    <div class="claim-percentage-num">
                      待结算:{{ item.policeSettlementPriceWait || '-' }}
                    </div>
                  </div>
                </div>
                <div class="service-money-r-item mb-50">
                  ...
                  <div class="service-money-r-item-b">
                    <div class="icon-ok" />
                    <div class="claim-percentage-text">
                      已结算:{{ item.claimSettlementPriceComplete || '-' }}
                    </div>
                    <div class="icon-money" />
                    <div class="claim-percentage-num">
                      待结算{{ item.claimSettlementPriceWait || '-' }}
                    </div>
                  </div>
                </div>
                <div class="service-money-r-item">
                  ...
                  <div class="service-money-r-item-b">
                    <div class="icon-ok" />
                    <div class="claim-percentage-text">
                      已结算:{{ item.supplierSettlementPriceComplete || '-' }}
                    </div>
                    <div class="icon-money" />
                    <div class="claim-percentage-num">
                      待结算:{{ item.supplierSettlementPriceWait || '-' }}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="policy-service-table">
              ...
              >
                ...
                <el-table-column label="服务项" show-overflow-tooltip>
                  <template slot-scope="{ row }">
                    <div class="table-maintext">
...
export default {
  ...
  created() {
    ...
    this.getCompanyData(params).then(res => {
      this.tableLoading = false;
      this.data = res.data || {};
      // if(this.data&&this.data.reportFormInsuranceProductVOList&&this.data.reportFormInsuranceProductVOList.length>0){
      //     this.data.reportFormInsuranceProductVOList.map(item=>{
      //         item.per1 = item.policeSettlementPriceComplete+item.policeSettlementPriceWait==0?0:this.getPercentage(item.policeSettlementPriceComplete,item.policeSettlementPriceWait)
      //         item.per2 = item.claimSettlementPriceComplete+item.claimSettlementPriceWait==0?0:this.getPercentage(item.claimSettlementPriceComplete,item.claimSettlementPriceWait)
      //         item.per3 = item.supplierSettlementPriceComplete+item.supplierSettlementPriceWait==0?0:this.getPercentage(item.supplierSettlementPriceComplete,item.supplierSettlementPriceWait)
      //     })
      // }
    });
  },
  methods: {
    ...mapActions('settle', ['getCompanyData']),
    cellStyle(obj) {
