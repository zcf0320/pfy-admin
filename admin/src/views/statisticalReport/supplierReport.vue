  ...
  <div class="policy-report-page">
    <el-container>
      <el-scrollbar
        ...
        :native="false"
        :wrap-class="`el-main ${data.length <= 0 ? '' : 'spacing'}`"
        tag="main"
        :noresize="false"
      >
        <div
        ...
        >
          ...
          <div class="company-item">
            <div class="policy-service-table">
              <el-table
                :data="item.serviceInfoFinancials"
                  ...
                  <template slot-scope="{ row }">
                    <div class="table-maintext">
                      <div
                        ...
                        @click="goPath(row.serviceInfoType, item.serviceInfoId)"
                      >
                        {{ row.serviceInfoType || '-' }}
                      </div>
                    </div>
        ...
        <div v-if="data.length <= 0" class="title-top">暂无数据</div>
      </el-scrollbar>
    </el-container>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch('settle/getSupplierData', params).then(res => {
      this.data = res.data;
  ...
  methods: {
    ...
    goPath(obj, serviceInfoId) {
      //   let showNo = obj.showNo
      if (obj === '供应商结算') {
        // 供应商结算
        this.$router.push({
          path: `/financial-center/service-item-settlement-details?no=${serviceInfoId}`
        });
      } else if (obj === '理赔结算') {
        // 理赔结算
        this.$router.push({
          path: `/financial-center/claim-settlement-details?no=${serviceInfoId}`
        });
      } else {
        // 保单结算
        this.$router.push({
