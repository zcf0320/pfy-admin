        ...
        <div class="details-content flex-col">
          ...
          <div class="scroll-table">
                ...
                >
                  ...
                  <div class="data-list">
                    ...
                    <div class="data-item">
                      <el-row :gutter="20">
                        <el-col :span="12">
                          <el-form-item label="联系人" prop="chargePeople">
                            <el-input
                  ...
                  >
                    ...
                    <el-table-column label="简介">
                      <template slot-scope="scope">
                        <span
                          slot="reference"
                          :title="scope.row.serviceDescribe"
                          >{{ scope.row.serviceDescribe || '-' }}</span
                        >
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="200">
                      <template slot-scope="scope">
                        <span
                          v-if="
                            scope.row.cooperationStatus !== 2 &&
                              pageAuthority.includes('edit')
                          "
                          style="margin-right: 20px"
                          class="primary pointer"
                          @click="
                            queryUsersAndPackages(
                              scope.row.id,
                              scope.row.serviceId,
                            )
                          "
                        ...
                        <span
                          v-if="pageAuthority.includes('detail')"
                          class="primary pointer"
                          @click="lookServiceItemDetail(scope.row.id)"
                          >查看</span
                        >
                      </template>
                    </el-table-column>
            ...
            <div v-if="activeName === '3'">
              <Logs :id="baseForm.id" :type="'2'" :has-o-a-log="true" />
            </div>
            <div v-show="activeName === '5'">
              <PostageSet
                ref="postageSet"
                :company-mail-vo="baseForm.companyMailVo"
                :edit="edit"
              />
            </div>
            <ContractManage v-if="activeName === '4'" />
          </div>
        </div>
        <div
          v-if="
            (activeName === '0' || activeName === '1' || activeName === '5') &&
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          ...
          size="small"
          @click="submit"
          >{{ isPass ? '确 定' : '去停止合作' }}</el-button
        >
        <el-button
          size="small"
          style="border: 1px solid #3599fe; color: #3599fe"
    ...
    >
      <div>
        <Tab
          :btns="btnsDialog"
          :btn-style="{ overflow: 'visible' }"
          :current-key="tabCurrentKey"
          @change="dialogClick"
        />
        <el-table v-loading="tableLoading" :data="tableData || []">
          <el-table-column label="ID" min-width="180">
            <template slot-scope="scope">
              <span v-if="activeTab === '0'">{{ scope.row.id || '-' }}</span>
        ...
        <Pagination
          ...
          @pagination="getList"
        />
        <span class="dialog-red"
          >停止合作后将不会有生成新的预约单，已有权益的用户可正常使用，请谨慎操作。</span
        >
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button
...
import PostageSet from './component/postageSet';
import {
  serviceType,
  operationType,
  menuType,
...
export default {
  ...
  data() {
    ...
    const checkDistinctName = (rule, value, callback) => {
      if (!this.baseForm.id) {
        ...
        this.$store
          .dispatch('supplier/checkDistinctName', {
            id:
          ...
          .then(res => {
            if (!res.data.isDuplicate) {
              return callback(new Error('供应商名称重复'));
            } else if (!res.data.isLegal) {
              return callback(new Error('该企业不存在'));
            }
            callback();
          });
      } else {
    ...
    const checkDistinctSimpleName = (rule, value, callback) => {
      ...
      this.baseForm.id ? (baseId = this.baseForm.id) : (baseId = null);
      this.$store
        .dispatch('supplier/checkDistinctSimpleName', {
          id: this.$route.params.id === 'add' ? baseId : this.$route.params.id,
          simpleName: value
        })
        .then(res => {
          if (res && !res.data) {
            return callback(new Error('供应商简称重复'));
          }
          callback();
        });
    };
  ...
  methods: {
    ...
    dialogClick(tab) {
      ...
      if (tab === '0') {
        this.getRelateUsers(this.serviceInfoId, { page: 1 });
      } else {
        this.getRelatePackages(this.serviceInfoId, { page: 1 });
      }
    },
    // 新建证件弹框确认
    ...
    addOrUpdateSupplier(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.loading = true;
          if (this.baseForm.id) {
            if (this.baseForm.companyType === 6) {
              if (!this.$refs.postageSet.save()) {
                this.$message.warning('设置信息未完善！');
                this.loading = false;
                return;
              }
            }
            this.$store
              .dispatch('supplier/updateProvider', {
                ...this.baseForm,
                companyEstablishTime: this.baseForm.companyEstablishTime
                  ? moment(this.baseForm.companyEstablishTime).format(
                      'YYYY-MM-DD',
                    )
                  : null
              })
    ...
    getSupplierDetails(id) {
      const vm = this;
      vm.$store
        .dispatch('supplier/getProviderDetail', {
          id
        })
        .then(res => {
          vm.baseForm = res.obj;
          if (!res.obj.companyMailVo) {
            vm.baseForm.companyMailVo = {
              beyondPinkagePrice: 0,
              beyondPrescriptionFeePrice: 0,
              companyId: '',
              mailType: 2,
          ...
          vm.companyTypeChange(vm.baseForm.companyType);
    ...
    changeStatus(accountStatus) {
      ...
      if (accountStatus === 2) {
        this.$store.dispatch('supplier/enableProvider', params).then(() => {
          this.$message({
            message: '启用成功！',
            type: 'success'
          });
        ...
        this.$store.dispatch('supplier/releaseProvider', params).then(() => {
          this.$message({
            message: '解约成功！',
            type: 'success'
          });
    ...
    viewDetail(id) {
      let url = '';
      if (this.activeTab === '0') {
        url = `/operateManage/userManage/details/${id}`;
      } else {
        url = `/supplyChainManage/servicePackage/details/${id}/detail`;
      }
      const routeData = this.$router.resolve({
        path: url
      });
    ...
    rescind() {
      ...
      this.$store
        .dispatch('supplier/queryUsersAndPackages', params)
        .then(() => {
          this.queryUsersAndPackages();
        });
    },
    stopCopr() {
      // 1正常，2停止合作
      ...
      this.$store.dispatch('supplier/updateCorpStatus', params).then(() => {
        this.$message({
          message: `停止合作成功！`,
          type: 'success'
        });
    ...
    checkRescind() {
      let flag = false;
      if (this.baseForm.serviceInfos) {
        const data = this.baseForm.serviceInfos;
        for (const i in data) {
          if (data[i].cooperationStatus !== 2) {
            flag = true;
          }
        }
        flag ? (this.isPass = false) : (this.isPass = true);
      } else {
        this.isPass = true;
      }
      this.isVisible = true;
    },
    queryUsersAndPackages(id) {
      ...
      this.$store
        ...
        .then(res => {
          this.num = res.data;
          this.btnsDialog[0].name = `涉及用户（${res.data.users}）`;
          this.btnsDialog[1].name = `涉及服务包（${res.data.packages}）`;
          this.getRelateUsers(id, { page: 1 });
          this.activeTab = '0';
          this.tabCurrentKey = '0';
          this.isShow = true;
        });
    },
    getRelateUsers(id, pageInfo) {
      const { page } = pageInfo;
      const params = {
        serviceInfoId: id,
        pageSize: 5,
      ...
      this.$store.dispatch('supplier/getRelateUsers', params).then(res => {
        this.userList = res.records;
        this.tableData = this.userList;
        this.pagingOptions.pageNum = params.pageNum;
        this.pagingOptions.total = res.totalNum;
        this.tableData =
          this.activeTab === '0' ? this.userList : this.packageList;
      });
    },
    getRelatePackages(id, pageInfo) {
      const { page } = pageInfo;
      const params = {
        serviceInfoId: id,
        pageSize: 5,
      ...
      this.$store.dispatch('supplier/getRelatePackages', params).then(res => {
    ...
    getList() {
      this.activeTab === 0
        ? this.getRelateUsers(this.serviceInfoId, {
            page: this.pagingOptions.pageNum
          })
        : this.getRelatePackages(this.serviceInfoId, {
            page: this.pagingOptions.pageNum
          });
    },
    formatAccout(val) {
